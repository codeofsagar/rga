...
export default function AdminDashboard() {
  ...
  const handleEventSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setIsSubmitting(true);
    try { const eventData = { ...eventForm, price: Number(eventForm.price), capacity: Number(eventForm.capacity), status: editingEventId ? events.find(e => e.id === editingEventId)?.status : 'active' };
  ...
  const handleGallerySubmit = async (e: React.FormEvent) => {
    ...
    try { let url = previewUrl; if (selectedFile) { const storageRef = ref(storage, `gallery/${Date.now()}_${selectedFile.name}`); await uploadBytes(storageRef, selectedFile); url = await getDownloadURL(storageRef); }
